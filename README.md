...
- 修复主窗口最小化后，通过全局快捷键或托盘重新唤起时窗口闪现后又消失的问题
- 为外部唤起主窗口增加防抖与失焦隐藏抑制，恢复最小化窗口时行为更稳定
- 修复打包后应用图标显示为空白的问题，恢复资源访问范围与 CSP 配置
- 搜索结果启动应用后立即清空搜索输入框，避免残留关键字影响下一次使用
- 提供独立可执行文件、MSI 安装包和 NSIS 安装包

### 1.0.7

- 将应用标题和图标移动到窗口标题栏，并与左侧分类区域对齐
- 优化右上角窗口控制按钮位置与尺寸，使按钮贴近窗口顶部
- 顶部添加按钮改为更紧凑的纯 `+` 图标
